...
get_active_environment() {
    # Verificar qual ambiente está ativo através do Traefik
    local active_env="green"  # padrão
    
    if command -v curl >/dev/null 2>&1; then
        # Tentar determinar através das labels do Traefik
        local traefik_config
        traefik_config=$(curl -s "$TRAEFIK_API" 2>/dev/null || echo "{}")
        
        if echo "$traefik_config" | grep -q "backend-blue"; then
            if echo "$traefik_config" | grep -q "traefik.http.routers.backend.service=backend-blue"; then
                active_env="blue"
            fi
        fi
    fi
...
switch_traffic() {
    ...
    # Atualizar labels do Traefik via docker-compose
    cd "$PROJECT_ROOT"
    
    # Criar arquivo temporário com as novas configurações
    local temp_compose="docker-compose.switch.yml"
    
    # Copiar configuração atual e modificar as labels do Traefik
    cp docker-compose.prod.yml "$temp_compose"
    
    # Atualizar as labels para apontar para o novo ambiente
    if [[ "$new_env" == "blue" ]]; then
        sed -i '' 's/traefik.http.routers.backend.service=backend-green/traefik.http.routers.backend.service=backend-blue/g' "$temp_compose"
    else
        sed -i '' 's/traefik.http.routers.backend.service=backend-blue/traefik.http.routers.backend.service=backend-green/g' "$temp_compose"
    fi
    
    # Aplicar as mudanças
    docker-compose -f "$temp_compose" up -d traefik
